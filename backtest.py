"""7-day historical backtest for Trade Travel Chill strategy.

Fetches historical candles for all quality-filtered pairs, steps through
time at 1H intervals, runs the full strategy pipeline, and simulates
entries/exits with proper SL/TP and trailing stop logic.
...
BACKTEST_DAYS = 7
WARMUP_1H = 80              # Extra 1H candles before backtest window for analyzer warmup
...
class BacktestEngine:
    def __init__(self) -> None:
        ...
        self.positions: list[BacktestPosition] = []  # open
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            print(f"Found {len(pairs)} quality futures pairs")

            # Fetch all candles
            all_candles = await self._fetch_all_candles(pairs)
            valid_pairs = [p for p in pairs if p in all_candles and "1h" in all_candles[p]]
            print(f"Fetched candles for {len(valid_pairs)} pairs")

            # Determine backtest window from 1H candles
            now = datetime.now(timezone.utc)
            bt_start = now - timedelta(days=BACKTEST_DAYS)

            # Get all 1H timestamps in the backtest window (union across all pairs)
            ...
            print(f"Stepping through {len(all_times_sorted)} hourly candles ({bt_start.strftime('%b %d')} → {now.strftime('%b %d')})")
            print()
            ...
            for i, current_time in enumerate(all_times_sorted):
                ...
                for sym in valid_pairs:
                    ...
                    signals_generated += 1
                    self._try_enter(signal, current_time)
            ...
            # Force close remaining positions at last candle close
            if all_times_sorted:
    ...
    async def _fetch_all_candles(self, pairs: list[str]) -> dict:
        """Fetch 1H/4H/1D candles for all pairs with rate limiting."""
        all_candles: dict = {}
        total = len(pairs)
        fetched = 0
        ...
        since_4h = now_ms - (BACKTEST_DAYS * 24 + 200) * 3600 * 1000  # ~200 4H candles
        since_1d = now_ms - 60 * 86400 * 1000  # ~60 days
        ...
        for batch_start in range(0, total, BATCH_SIZE):
            ...
            tasks = []
            for sym in batch:
                tasks.append(self._fetch_pair_candles(sym, since_1h, since_4h, since_1d))

            results = await asyncio.gather(*tasks, return_exceptions=True)

            for sym, result in zip(batch, results):
                if isinstance(result, Exception):
                    print(f"  Failed to fetch {sym}: {result}")
                    continue
                if result:
    ...
    def _analyze_at_time(self, symbol: str, candles: dict, current_time: datetime):
        """Run the full strategy pipeline at a given point in time."""
        ...
        # Slice to current_time (inclusive)
        ...
        # Attach sweep and ATR for SL/TP calc
    ...
    def _try_enter(self, signal, current_time: datetime) -> None:
        """Attempt to enter a position from a signal."""
        ...
        # SL calculation (same as orders.py)
        sl = None
        ...
        if is_long:
            min_tp = entry + min_tp_dist
            tp = min_tp
            if sweep and sweep.target_level and sweep.target_level > entry:
                tp = max(sweep.target_level, min_tp)
            # Check structural levels
            for key in ["1h_swing_high", "4h_swing_high", "1d_swing_high"]:
                level = signal.key_levels.get(key)
            ...
            tp = min_tp
            if sweep and sweep.target_level and sweep.target_level < entry:
                tp = min(sweep.target_level, min_tp)
            for key in ["1h_swing_low", "4h_swing_low", "1d_swing_low"]:
                level = signal.key_levels.get(key)
                if level and level <= min_tp:
                    tp = level
                    break
    ...
    def _monitor_positions(self, current_time: datetime, all_candles: dict) -> None:
        """Check all open positions against current candle."""
        for pos in list(self.positions):
            df = all_candles.get(pos.symbol, {}).get("1h")
    ...
    def _equity(self, current_time: datetime, all_candles: dict) -> float:
        ...
        for pos in self.positions:
            ...
            if df is None:
                continue
            # Find last available price at or before current_time
    ...
    def _print_report(self, signals_generated: int, pairs_scanned: int, bt_start: datetime, bt_end: datetime) -> None:
        print("=" * 90)
        print(f"  BACKTEST RESULTS: {bt_start.strftime('%b %d')} - {bt_end.strftime('%b %d, %Y')} ({BACKTEST_DAYS} days)")
        print("=" * 90)
        print()
    ...
    def _save_results(self, signals_generated: int, pairs_scanned: int, bt_start: datetime, bt_end: datetime) -> None:
        """Save backtest results to JSON for dashboard comparison."""
        ...
        filename = f"backtest_results/original_{ts}.json"
