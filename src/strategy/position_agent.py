...
## MM Method Position Management Rules

If MM Method data is provided, use it as HIGH-WEIGHT guidance for SL and TP decisions.

### SL Management Through Levels
- NEVER tighten SL just to improve R:R — SL goes where structure demands
...
- Once Level 2 running: can place SL just under the 50 EMA
- At each level progression, consider tightening SL to lock in the previous level's gains

### Take Profit by Level Count
- **Level 1 target:** 50 EMA (counter-trend), then 200 EMA. Take 25-33% off
- **Level 2 target:** 800 EMA or previous unrecovered Vector candle. Take 50% total
- **Level 3 target:** Higher TF EMA (200/800 on higher TF) or previous HOW/LOW. Take remaining
- At Level 3: look for Stopping Volume Candle (small body, large wick, high volume) — confirmed exhaustion

### Friday UK Session Exit Rule
- Friday UK session (3:30am-9am NY): exit when peak formation appears during UK close
- If only 2 levels completed by Friday: can hold through weekend ONLY if SL can be moved without ruining R:R

### The Refund Zone
- If price CLOSES below wick of 2nd peak W (or above for M): formation invalidated
- This is the tightest possible SL — use for TIGHTEN_SL when structure breaks

### Extended Rise/Drop (Level 4+)
- A 4th level almost always brings correction — if you see Level 4, strongly consider not extending TP3
- Volume degradation at Level 3 (decreasing volume on successive candles) = guaranteed pause, likely reversal

### EMA-Based SL Tightening
- After Level 1 board meeting: tighten SL to just below 50 EMA (longs) or above (shorts)
- After Level 2: tighten SL to protect at least Level 1 profits
- EMAs flattening + Stopping Volume Candle = trend ending, tighten aggressively"""
...
**IMPORTANT: If position management lessons or MM Method context are shown above, actively apply them. These come from real past trades and institutional cycle analysis.**
...
Evaluate this position and respond with your JSON decision. Default to HOLD unless you see clear evidence for action."""
    ...
    def _format_mm_method_context(ctx: dict) -> str:
        """Format MM Method context for Agent 3's position management evaluation.

        The mm_method dict comes from the trade's stored signal context.
        Nested keys match scanner output:
        ...
        lines = ["### MM Method Position Context"]
        ...
        if phase:
            ...
            if wc.get("take_profit_signal"):
                lines.append("  NOTE: Take profit signal active — consider tightening SL or taking profit")

        # Level count — critical for SL/TP decisions
        levels = mm.get("levels", {})
        level = levels.get("current") if levels else None
        if level is not None:
            ext = " (EXTENDED — 4th level, correction imminent)" if levels.get("is_extended") else ""
            lines.append(f"- **Level Count:** {level} / 3{ext}")
            if level == 1:
                lines.append("  Level 1 complete — consider tightening SL to entry (breakeven) or above Level 1 TP")
                lines.append("  Do NOT move SL to breakeven before Level 2 starts")
            elif level == 2:
                lines.append("  Level 2 running — can tighten SL to just under 50 EMA")
                lines.append("  Consider taking partial profit (50% total)")
            elif level >= 3:
                lines.append("  Level 3+ reached — look for Stopping Volume Candle")
                lines.append("  Take remaining profit at Level 3 target. Do NOT extend TP3 for Level 4+")
        ...
        if levels and (levels.get("svc_detected") or levels.get("svc_confirmed")):
            ...
            lines.append(f"- **STOPPING VOLUME CANDLE {confirmed}** — trend exhaustion")
            lines.append("  ACTION: Tighten SL aggressively. Do NOT extend TP3.")

        # Volume degradation
        if levels and levels.get("volume_degrading"):
            lines.append("- **Volume Degradation** — momentum fading, expect pause or reversal")

        # Formation
        formation = mm.get("formation")
        if formation and formation.get("type"):
            lines.append(f"- **Active Formation:** {formation['type']} (direction: {formation.get('direction', '?')})")

        # EMA state for SL placement
        ema = mm.get("ema", {})
        if ema:
            ema_parts = []
            ...
            if ema.get("price_dist_200") is not None:
                direction = "above" if ema["price_dist_200"] > 0 else "below"
                ema_parts.append(f"{abs(ema['price_dist_200']):.2f}% {direction} 200 EMA")
            if ema_parts:
                lines.append(f"- **EMA Distance:** {' | '.join(ema_parts)}")

        # Trend flattening = trend ending
        trend = mm.get("trend", {})
        if trend:
            if trend.get("is_flattening"):
                lines.append("- **Trend FLATTENING** — EMAs converging, trend may be ending. Tighten SL.")
            elif trend.get("is_accelerating"):
                lines.append("- **Trend ACCELERATING** — momentum supports continuation. TP3 extension may be valid.")

        # Session — Friday exit rule
        session = mm.get("session", {})
        if session and session.get("name"):
            session_name = session["name"]
            lines.append(f"- **Session:** {session_name}")
            # Friday detection from session name
            name_lower = session_name.lower() if session_name else ""
            if "friday" in name_lower and "uk" in name_lower:
                lines.append("  FRIDAY UK SESSION: Look for peak formation to exit. Take profit by UK close (9am NY)")
        ...
        if wc:
            ...
            if how or low:
                target_parts = []
                if how:
                    target_parts.append(f"HOW: {how:.6g}")
                if low:
                    target_parts.append(f"LOW: {low:.6g}")
                lines.append(f"- **Weekly Range Targets:** {' | '.join(target_parts)}")

        # Level-based targets (for TP decisions)
        ...
        if targets:
            ...
            if tgt_parts:
                lines.append(f"- **MM Targets:** {' | '.join(tgt_parts)}")
            uvecs = targets.get("unrecovered_vectors", 0)
            if uvecs:
                lines.append(f"  {uvecs} unrecovered vector(s) — additional TP zones")

        # Confluence
        conf = mm.get("confluence", {})
        if conf and conf.get("grade"):
            lines.append(f"- **MM Confluence Grade:** {conf['grade']}")
